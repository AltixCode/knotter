...
describe("Paywall", () => {
  ...
  it("lists the leading claims as a badge cluster, title only", async () => {
    const { getByText, queryByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("feat1Title"))).toBeTruthy();
    expect(getByText(t("feat2Title"))).toBeTruthy();
    expect(getByText(t("feat3Title"))).toBeTruthy();
    // The cluster badges are icon + short label only — no long-form
    // description rendered in that region of the structure.
    expect(queryByText(t("feat1Desc"))).toBeNull();
  });

  it("gives the last claim its own hero card, title and full description", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
